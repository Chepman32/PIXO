...
@objc(SquozeImageConverter)
class SquozeImageConverter: NSObject {
  ...
  private func normalizedURL(from input: String) throws -> URL {
    ...
    if value.isEmpty {
      throw NSError(domain: "SquozeImageConverter", code: 1002, userInfo: [NSLocalizedDescriptionKey: "Source path is empty"])
    }
  ...
  private func makeOutputURL(format: String) throws -> URL {
    ...
    let root = fileManager.temporaryDirectory.appendingPathComponent("squoze-output", isDirectory: true)

    if !fileManager.fileExists(atPath: root.path) {
      try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
    }

    let fileName = "squoze-\(UUID().uuidString.lowercased()).\(format.lowercased())"
